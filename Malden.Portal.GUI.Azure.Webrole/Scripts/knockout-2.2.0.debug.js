...
ko.utils.compareArrays = (function () {
    ...
    function compareSmallArrayToBigArray(smlArray, bigArray, statusNotInSml, statusNotInBig, dontLimitMoves) {
        var myMin = Math.min,
            ...
            thisuserPurchase, lastuserPurchase,
            bigIndexMaxForuserPurchase, bigIndexMinForuserPurchase;

        for (smlIndex = 0; smlIndex <= smlIndexMax; smlIndex++) {
            lastuserPurchase = thisuserPurchase;
            editDistanceMatrix.push(thisuserPurchase = []);
            bigIndexMaxForuserPurchase = myMin(bigIndexMax, smlIndex + compareRange);
            bigIndexMinForuserPurchase = myMax(0, smlIndex - 1);
            for (bigIndex = bigIndexMinForuserPurchase; bigIndex <= bigIndexMaxForuserPurchase; bigIndex++) {
                if (!bigIndex)
                    thisuserPurchase[bigIndex] = smlIndex + 1;
                else if (!smlIndex)  // Top row - transform empty array into new array via additions
                    thisuserPurchase[bigIndex] = bigIndex + 1;
                else if (smlArray[smlIndex - 1] === bigArray[bigIndex - 1])
                    thisuserPurchase[bigIndex] = lastuserPurchase[bigIndex - 1];                  // copy value (no edit)
                else {
                    var northDistance = lastuserPurchase[bigIndex] || maxDistance;       // not in big (deletion)
                    var westDistance = thisuserPurchase[bigIndex - 1] || maxDistance;    // not in small (addition)
                    thisuserPurchase[bigIndex] = myMin(northDistance, westDistance) + 1;
                }
            }
        }
