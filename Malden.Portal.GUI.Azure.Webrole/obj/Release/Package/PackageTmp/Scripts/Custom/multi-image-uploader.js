    ...
    if (fileControl.files.length > 0) {
        ...
        for (var i = 0; i < fileControl.files.length; i++) {
            ...
            if (fileStatus) {
                ...
                $("#statusMessageLabel").text("Upload cancelled by user!");
            }
        }
    }
};
    ...
    $.ajax({
        ...
        switch (status) {
            case 0:
                fileStatus = true;
                break;
            case 1: alert("File already exists");
                fileStatus = false;
                break;
            case 2:
                alert("File already uploaded for the selected type and release");
    ...
    var chunkedFileUploader =
    {
        ...
        sendFile: function (uploader)
        {
            ...
            sendNextChunk = function () {
                ...
                jqxhr = $.ajax({
                    async: true,
                    url: ('/Release/UploadChunk?id=' + uploader.currentChunk + "&fileIndex=" + uploader.fileIndex),
                    data: fileChunk,
                    cache: false,
                    contentType: false,
                    processData: false,
